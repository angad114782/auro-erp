...
export async function getMaterialRequests(req, res) {
  try {
    ...
    const items = await PCMaterialRequest.find({ projectId }).sort({ createdAt: -1 }).lean();
...
export async function getMaterialRequestById(req, res) {
  try {
    ...
    const doc = await PCMaterialRequest.findOne({ _id: requestId, projectId }).lean();
...
export async function updateMaterialRequest(req, res) {
  try {
    const { projectId, requestId } = req.params;
    const body = req.body;
    const updated = await PCMaterialRequest.findOneAndUpdate({ _id: requestId, projectId }, { $set: body }, { new: true });
...
export async function deleteMaterialRequest(req, res) {
  try {
    const { projectId, requestId } = req.params;
    const removed = await PCMaterialRequest.findOneAndDelete({ _id: requestId, projectId });
