...
const statusAliases = [
  ...
  {
    canon: "po_approved",
    aliases: ["po approved", "poapproved", "po-appr", "po appr", "approved po"],
  },
  {
    canon: "delivery_pending",
    aliases: ["delivery pending", "pendingdelivery"],
  },
  {
    canon: "parcel_delivered",
    aliases: ["parcel delivered"],
  },
  {
    canon: "delivered",
    aliases: ["delivered"],
...
export function normalizeProjectStatus(input) {
  ...
  for (const row of statusAliases) {
    ...
    if (
      row.aliases.some((a) => a === s || a.replace(/\s+/g, "") === collapsed)
    ) {
      return row.canon;
    }
  }
...
const approvalAliases = [
  ...
  {
    canon: "update_req",
    aliases: ["update required", "update req", "need update"],
  },
...
export function requireValidClientApproval(input) {
  ...
  if (!norm) {
    ...
    const err = new Error(
      `Invalid client approval "${input}". Allowed: ${allowed}`
    );
    err.status = 400;
    throw err;
  }
