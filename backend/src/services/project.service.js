...
export const setProjectPO = async (id, payload = {}, by = null) => {
  ...
  const qty   = payload.orderQuantity != null ? Number(payload.orderQuantity) : null;
  ...
  const totalAmount = (effectiveQty != null && effectivePrice != null)
  ...
  project.po = {
    ...
    status:              nextStatus,
    deliveryDate:        payload.deliveryDate ? new Date(payload.deliveryDate) : (prev.deliveryDate ?? null),
    paymentTerms:        payload.paymentTerms        ?? (prev.paymentTerms        ?? ""),
    urgencyLevel:        payload.urgencyLevel        ?? (prev.urgencyLevel        ?? "Normal"),
    qualityRequirements: payload.qualityRequirements ?? (prev.qualityRequirements ?? ""),
    clientFeedback:      payload.clientFeedback      ?? (prev.clientFeedback      ?? ""),
    specialInstructions: payload.specialInstructions ?? (prev.specialInstructions ?? ""),
