...
export async function getCardById(cardId) {
  ...
  const resolvedName = isMeaningfulName(rawName)
    ? String(rawName).trim()
    : defaultProductName;
...
async function buildItemDeptMap(projectId) {
  ...
  const map = {};
  (upperCostRows || []).forEach((r) => {
    if (r._id) map[String(r._id)] = r.department || null;
  });
  (componentCostRows || []).forEach((r) => {
    if (r._id) map[String(r._id)] = r.department || null;
  });
  (materialCostRows || []).forEach((r) => {
    if (r._id) map[String(r._id)] = r.department || null;
  });
  (packagingCostRows || []).forEach((r) => {
    if (r._id) map[String(r._id)] = r.department || null;
  });
  (miscCostRows || []).forEach((r) => {
    if (r._id) map[String(r._id)] = r.department || null;
  });
...
export async function createMaterialRequestForCard(
  ...
  try {
    ...
    const upperSrc =
      Array.isArray(mrPayload.upper) && mrPayload.upper.length
        ? mrPayload.upper
        : Array.isArray(card.upper)
        ? card.upper
        : [];
    const materialsSrc =
    ...
    const componentsSrc =
      Array.isArray(mrPayload.components) && mrPayload.components.length
        ...
        : [];
    const packagingSrc =
      Array.isArray(mrPayload.packaging) && mrPayload.packaging.length
        ? mrPayload.packaging
        : Array.isArray(card.packaging)
        ? card.packaging
        : [];
    const miscSrc =
      Array.isArray(mrPayload.misc) && mrPayload.misc.length
        ? mrPayload.misc
        : Array.isArray(card.misc)
        ? card.misc
        : [];
      ...
      [
        {
          ...
          componentsSnapshot,
          notes:
            mrPayload.notes || "Material request created from production card",
        },
      ],
      { session }
    );
...
export async function listMaterialRequests(
  ...
  if (filter.projectId) matchStage.projectId = filter.projectId;
    ...
    pipeline.push({
      $match: {
        $or: [
          { cardNumber: regex },
          { requestedBy: regex },
          { "projectDetails.artName": regex },
          { "projectDetails.productName": regex },
        ],
      },
    });
  }
  ...
  try {
    ...
    if (options.syncCard && mr.productionCardId) {
      ...
      if (card) {
        ...
        if (Array.isArray(mr.packaging) && mr.packaging.length)
          card.packaging = mr.packaging;
        if (Array.isArray(mr.misc) && mr.misc.length) card.misc = mr.misc;

        // keep legacy snapshots (if provided)
        if (Array.isArray(mr.materialsSnapshot) && mr.materialsSnapshot.length)
          card.materialsSnapshot = mr.materialsSnapshot;
        if (
          Array.isArray(mr.componentsSnapshot) &&
          mr.componentsSnapshot.length
        )
          card.componentsSnapshot = mr.componentsSnapshot;
...
export async function getMaterialRequestById(mrId) {
  ...
  const mr = await PCMaterialRequest.findById(mrId)
    .populate({
      ...
      select:
        "_id cardNumber projectId productName cardQuantity assignedPlant upper materials components packaging misc materialsSnapshot componentsSnapshot",
      populate: { path: "assignedPlant", select: "name" },
    })
...
export async function updateCardStage(
  ...
  if (!newStage || typeof newStage !== "string")
    throw new Error("newStage required");
  ...
  const updated = await PCProductionCard.findOneAndUpdate(
    query,
    { $set: { stage: normalized }, $push: { stageHistory: historyEntry } },
    { new: true, runValidators: true }
  )
    .populate({ path: "assignedPlant", select: "name" })
    .populate({ path: "materialRequests", select: "_id status createdAt" });
...
/* ---------- getProjectTrackingOverview ---------- */
export async function getProjectTrackingOverview(
  projectId,
  opts = { page: 1, limit: 25, sort: "createdAt:desc" }
) {
  if (!projectId) throw new Error("projectId required");
  if (!mongoose.Types.ObjectId.isValid(String(projectId)))
    throw new Error("Invalid projectId");
  ...
  async function buildItemDeptMapOnce() {
    const [upperCostRows, componentCostRows] = await Promise.all([
      UpperCostRow.find({ projectId: projectObjId })
        .select("_id department")
        .lean()
        .catch(() => []),
      ComponentCostRow.find({ projectId: projectObjId })
        .select("_id department")
        .lean()
        .catch(() => []),
    ]);
  ...
  const trackingItems = (cards || []).map((c) => {
    ...
    const ensureDept = (row) => {
      ...
      if (row.itemId && itemDeptMap[String(row.itemId)])
        return { ...row, department: itemDeptMap[String(row.itemId)] };
    ...
    const finalProductName = isMeaningfulName(rawName)
      ? String(rawName).trim()
      : defaultProductName;
    ...
    return {
      ...
      materialRequestCount: Array.isArray(c.materialRequests)
        ? c.materialRequests.length
        : 0,
  ...
  const mapToSortedList = (mapObj) =>
    Object.values(mapObj || {}).sort((a, b) =>
  ...
  return {
    ...
    trackingCards: {
      ...
      limit,
      total: cardsTotal,
      pages: Math.max(1, Math.ceil(cardsTotal / limit)),
    },
  };
}
...
*/
export async function getTrackingMaterialsForProject(
  projectId,
  opts = { page: 1, limit: 200, department: null }
) {
  if (!projectId) throw new Error("projectId required");
  if (!mongoose.Types.ObjectId.isValid(String(projectId)))
    throw new Error("Invalid projectId");
  ...
  const itemDept = {};
  (upperCostRows || []).forEach((r) => {
    if (r._id) itemDept[String(r._id)] = r.department || null;
  });
  (componentCostRows || []).forEach((r) => {
    if (r._id) itemDept[String(r._id)] = r.department || null;
  });
  ...
  const [cards, totalCards] = await Promise.all([
    PCProductionCard.find({
      ...
      .select(
        "cardNumber productName cardQuantity assignedPlant upper materials components packaging misc materialRequestStatus materialsSnapshot componentsSnapshot"
      )
      .populate({ path: "assignedPlant", select: "name" })
      .sort({ createdAt: -1 })
      .skip(skip)
  ...
  const cardsWithSnapshots = (cards || []).map((c) => {
    ...
    const ensureDept = (row) => {
      if (row.department) return row;
      if (row.itemId && itemDept[String(row.itemId)])
        return { ...row, department: itemDept[String(row.itemId)] };
      return { ...row, department: row.department || "unknown" };
    };
    ...
    for (const r of matCombined) {
      ...
      const dept = r.department || "unknown";
      if (!aggregated.byDepartment[dept])
        aggregated.byDepartment[dept] = { materials: {}, components: {} };
    ...
    return {
      ...
      misc: miscNorm,
      materialsSnapshot: Array.isArray(c.materialsSnapshot)
        ? c.materialsSnapshot
        : [...upperNorm, ...materialsNorm],
      componentsSnapshot: Array.isArray(c.componentsSnapshot)
        ? c.componentsSnapshot
        : [...componentsNorm, ...packagingNorm, ...miscNorm],
    };
  });
  ...
  return {
    ...
    aggregated: {
      separated,
      overall: {
        materials: overallMaterialsList,
        components: overallComponentsList,
      },
      byDepartment: byDept,
    },
  };
 ...
 * }
 */
export async function getTrackingByDepartmentAcrossProjects(
  department,
  opts = { page: 1, limit: 200 }
) {
  ...
  const [cards, totalCards] = await Promise.all([
    PCProductionCard.find({ stage: "Tracking", isActive: true })
      .select(
        "cardNumber productName cardQuantity projectId upper materials components packaging misc materialRequestStatus"
      )
  ...
  const projIds = Array.from(
    new Set(
      (cards || [])
        .map((c) => String(c.projectId?._id || c.projectId))
        .filter(Boolean)
    )
  );
  ...
  async function buildItemDeptMapForProjects(projectIds) {
    ...
    const [upperRows, componentRows, materialRows, packagingRows, miscRows] =
      await Promise.all([
        UpperCostRow.find({ projectId: { $in: projectIds } })
          .select("_id projectId department")
          .lean()
          .catch(() => []),
        ComponentCostRow.find({ projectId: { $in: projectIds } })
          .select("_id projectId department")
          .lean()
          .catch(() => []),
        MaterialCostRow.find({ projectId: { $in: projectIds } })
          .select("_id projectId department")
          .lean()
          .catch(() => []),
        PackagingCostRow.find({ projectId: { $in: projectIds } })
          .select("_id projectId department")
          .lean()
          .catch(() => []),
        MiscCostRow.find({ projectId: { $in: projectIds } })
          .select("_id projectId department")
          .lean()
          .catch(() => []),
      ]);
    ...
    addRows(upperRows);
    addRows(componentRows);
    addRows(materialRows);
    addRows(packagingRows);
    addRows(miscRows);
    return mapByProject;
  }
  ...
  const ensureDept = (row, projectId) => {
    ...
    if (
      row.itemId &&
      itemDeptMapByProject[String(projectId)] &&
      itemDeptMapByProject[String(projectId)][String(row.itemId)]
    ) {
      return {
        ...row,
        department: itemDeptMapByProject[String(projectId)][String(row.itemId)],
      };
    }
    return { ...row, department: row.department ?? "unknown" };
  };
  ...
  for (const c of cards || []) {
    ...
    const up = (Array.isArray(c.upper) ? c.upper : []).map((r) =>
      ensureDept(r, projId)
    );
    const mats = (Array.isArray(c.materials) ? c.materials : []).map((r) =>
      ensureDept(r, projId)
    );
    const comps = (Array.isArray(c.components) ? c.components : []).map((r) =>
      ensureDept(r, projId)
    );
    const packs = (Array.isArray(c.packaging) ? c.packaging : []).map((r) =>
      ensureDept(r, projId)
    );
    const misc = (Array.isArray(c.misc) ? c.misc : []).map((r) =>
      ensureDept(r, projId)
    );
    ...
    const cardMaterialsForDept = matCombined.filter(
      (r) => (r.department || "unknown") === department
    );
    const cardComponentsForDept = compCombined.filter(
      (r) => (r.department || "unknown") === department
    );

    // skip if neither materials nor components for this department
    if (
      cardMaterialsForDept.length === 0 &&
      cardComponentsForDept.length === 0
    ) {
...
export async function getTrackingByDepartmentAcrossProjectsFlattened(
  department,
  opts = { page: 1, limit: 200 }
) {
  if (!department) throw new Error("department required");
  ...
  for (const c of cards || []) {
    ...
    const materialsForDept = matCombined.filter(
      (r) => (r.department || "unknown") === department
    );
    const componentsForDept = compCombined.filter(
      (r) => (r.department || "unknown") === department
    );

    // If nothing for this card in this department -> skip
    if (materialsForDept.length === 0 && componentsForDept.length === 0)
      continue;
