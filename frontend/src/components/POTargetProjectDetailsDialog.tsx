...
export function POPendingProjectDetailsDialog(props: Props) {
  ...
  const [activeTab, setActiveTab] = useState<
    "details" | "materials" | "feedback" | "timeline"
  >("details");
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      await generateProjectPDF({
        ...
        colorVariants: colorVariantsObj, // No color variants for PO Pending
      });
  ...
  const convertColorVariants = useCallback(
    (projectData: any): Map<string, any> => {
  ...
  // Load color variants from project
  useEffect(() => {
    if (project && open) {
  ...
  // Load cost data based on active color variant
  const loadCostData = useCallback(
    async (color: string) => {
      ...
      try {
        // First check if we have color variant data
        if (localColorVariants && localColorVariants.size > 0 && color) {
          const variant = localColorVariants.get(color);
          if (variant?.costing) {
            setCostData({
        ...
        // Fallback to default API calls for the main color
        if (color === project.color || !color) {
          ...
          setCostData({
            ...
            misc: miscRes.data.rows || [],
            labour: labourRes.data.labour || { items: [], directTotal: 0 },
            summary: summaryRes.data.hasCostData
    ...
    [project, localColorVariants]
  );

  // Load cost data when active color tab changes
  useEffect(() => {
    if (activeColorTab && project?._id) {
      loadCostData(activeColorTab);
    }
  }, [activeColorTab, project?._id, loadCostData]);
  ...
  const handleColorVariantsSave = useCallback(
    ...
    [project, reloadProjects, convertColorVariants]
  );
        ...
        >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge
                      className={`${currentStage.color} text-xs px-2 py-1`}
                    >
                      {currentStage.name}
                    </Badge>
                    <Badge
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide">
            <div className="px-4 md:px-8 py-4 md:py-8 space-y-6 md:space-y-10">
              ...
              {/* Materials & Components Analysis - Color Variant Tab System */}
              <div className="space-y-4 md:space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  {(colorVariantTabs.length > 0 ||
                    (project?.color && activeColorTab === project.color)) &&
                    (() => {
                      if (isLoadingCost) {
                        return (
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Right Column - Labour Table and PO Summary */}
                          <div className="space-y-4 md:space-y-6">
                            {hasLabour && (
                              <LabourTable labour={costData.labour} />
                            )}

                            {/* If no labour data, show empty state */}
                            {!hasLabour &&
                              costTableCount === 0 &&
                              useTwoColumns && (
