...
export function MasterDataManagement({
  ...
  const { rdProjects, brands, categories, types, colors, countries, users } =
    useERPStore();
  ...
  const getBrandName = (brandId: string) =>
    brands.find((b) => b.id === brandId)?.brandName || "Unknown";
  const getCategoryName = (categoryId: string) =>
    categories.find((c) => c.id === categoryId)?.categoryName || "Unknown";
  const getTypeName = (typeId: string) =>
    types.find((t) => t.id === typeId)?.typeName || "Unknown";
  const getColorName = (colorId: string) =>
    colors.find((c) => c.id === colorId)?.colorName || "Unknown";
  const getCountryName = (countryId: string) =>
    countries.find((c) => c.id === countryId)?.countryName || "Unknown";
  const getDesignerName = (designerId: string) =>
    users.find((u) => u.id === designerId)?.userName || "Unknown";

  // Filter projects based on search term
  const filteredProjects = rdProjects.filter(
    (project) =>
  ...
  const avgProjectDuration =
    totalProjects > 0
      ? Math.round(
          filteredProjects.reduce((sum, p) => sum + p.duration, 0) /
            totalProjects
        )
      : 0;
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Complete product lifecycle tracking from R&D to production
              </p>
            </div>
          </div>
                        ...
                        <div className="space-y-1 text-xs">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                              ₹{project.tentativeCost.toLocaleString()}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Target:</span>
                            <span className="font-medium">
                              ₹{project.targetCost.toLocaleString()}
                            </span>
                          </div>
                          <div className="flex justify-between">
