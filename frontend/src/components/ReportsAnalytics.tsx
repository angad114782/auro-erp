...
export function ReportsAnalytics({ currentSubModule }: ReportsAnalyticsProps) {
  ...
  const [selectedSubModule, setSelectedSubModule] = useState(
    currentSubModule || "prototype-reports"
  );
  ...
  const getProjectsByStage = () => {
    ...
    return stages.map((stage) => ({
      ...
      percentage:
        (rdProjects.filter((p) => p.stage === stage).length /
          rdProjects.length) *
        100,
    }));
  };
  ...
  const getDesignerStats = () => {
    ...
    return designers.map((designer) => {
      ...
      const assignedProjects = rdProjects.filter(
        (p) => p.assignedTo === designerName
      );
      const completedProjects = assignedProjects.filter(
        (p) => p.stage === "Final Approved"
      );
      return {
        ...designer,
        name: designerName,
        ...
        efficiency:
          assignedProjects.length > 0
            ? (completedProjects.length / assignedProjects.length) * 100
            : 0,
      };
    });
  };
  ...
  const getBrandCostAnalysis = () => {
    return brands.map((brand) => {
      ...
      const totalCost = brandProjects.reduce(
        (sum, p) => sum + p.estimatedCost,
        0
      );
      const avgCost =
        brandProjects.length > 0 ? totalCost / brandProjects.length : 0;
      return {
        ...brand,
        name: brandName,
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-semibold text-gray-900">
                  {
                    rdProjects.filter((p) => p.stage === "Final Approved")
                      .length
                  }
                </p>
              </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-semibold text-gray-900">
                  {
                    rdProjects.filter(
                      (p) => !["Final Approved", "PO Issued"].includes(p.stage)
                    ).length
                  }
                </p>
              </div>
                ...
                {getDesignerStats().map((designer, index) => (
                  <tr key={designer.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-linear-to-br from-[#0c9dcb] to-[#26b4e0] rounded-full flex items-center justify-center text-white font-semibold text-sm">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className="w-16">
                          <Progress
                            value={designer.efficiency}
                            className="h-2"
                          />
                        </div>
                        <span className="text-sm text-gray-600">
                          {designer.efficiency.toFixed(1)}%
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <Badge
                        variant={
                          designer.efficiency >= 80
                            ? "default"
                            : designer.efficiency >= 60
                            ? "secondary"
                            : "destructive"
                        }
                      >
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {getBrandCostAnalysis().map((brand, index) => {
                  const totalBudget = getBrandCostAnalysis().reduce(
                    (sum, b) => sum + b.totalCost,
                    0
                  );
                  const percentage =
                    totalBudget > 0 ? (brand.totalCost / totalBudget) * 100 : 0;
                  return (
                    <tr key={brand.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-linear-to-br from-purple-500 to-purple-600 rounded-lg flex items-center justify-center text-white font-semibold text-sm">
                            {brand.name.charAt(0)}
                          </div>
                          <p className="font-medium text-gray-900">
                            {brand.name}
    ...
    <div className="space-y-6">
      <div className="text-center py-20">
        ...
        <h3 className="text-xl font-semibold text-gray-900 mb-2">
          Plant Utilization Reports
        </h3>
        <p className="text-gray-500 max-w-md mx-auto">
          Detailed plant utilization and efficiency reports will be available
          once production data is integrated.
        </p>
      </div>
    </div>
  );
          ...
          >
            <CardContent className="p-6">
              <div className="flex items-center gap-3 mb-3">
                <div
                  className={`w-10 h-10 ${module.color} rounded-lg flex items-center justify-center text-white`}
                >
                  {module.icon}
                </div>
        ...
        <CardHeader className="bg-linear-to-r from-gray-50 to-gray-100 rounded-t-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div
                className={`w-8 h-8 ${
                  subModules.find((m) => m.id === selectedSubModule)?.color
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  {
                    subModules.find((m) => m.id === selectedSubModule)
                      ?.description
                  }
                </p>
              </div>
