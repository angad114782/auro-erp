...
export function RDDashboard({ onNavigate }: RDDashboardProps) {
  const { rdProjects, brands, categories, types, colors, users } =
    useERPStore();
  ...
  const analytics = useMemo(() => {
    ...
    const redSealOK = rdProjects.filter(
      (p) => p.status === "Red Seal" && p.clientFeedback === "OK"
    );
    const redSealPending = rdProjects.filter(
      (p) => p.status === "Red Seal" && p.clientFeedback !== "OK"
    );
    const greenSealOK = rdProjects.filter(
      (p) => p.status === "Green Seal" && p.clientFeedback === "OK"
    );
    const greenSealPending = rdProjects.filter(
      (p) => p.status === "Green Seal" && p.clientFeedback !== "OK"
    );
    const poApproved = rdProjects.filter(
      (p) => p.status === "Final Approved" && p.poReceived
    );
    const poPending = rdProjects.filter(
      (p) => p.status === "Final Approved" && !p.poReceived
    );
    ...
    const stages = [
      {
        ...
        count: rdProjects.filter((p) => p.status === "Idea Submitted").length,
        color: "#0c9dcb",
      },
      {
        stage: "Costing",
        count: rdProjects.filter((p) =>
          ["Costing Pending", "Costing Received"].includes(p.status)
        ).length,
        color: "#ffc107",
      },
      {
      ...
      {
        stage: "Red Seal",
        count: rdProjects.filter((p) => p.status === "Red Seal").length,
        color: "#dc3545",
      },
      {
        stage: "Green Seal",
        count: rdProjects.filter((p) => p.status === "Green Seal").length,
        color: "#28a745",
      },
      {
        stage: "Approved",
    ...
    const pendingActions = [
      {
        ...
        count: rdProjects.filter((p) => p.status === "Costing Pending").length,
        icon: Clock,
      },
      {
