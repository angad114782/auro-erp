...
export function DeliveryManagement({ currentSubModule }: DeliveryManagementProps) {
  const { deliveryItems, updateDeliveryItem, addDeliveryItem, deleteDeliveryItem } = useERPStore();
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedDelivery, setSelectedDelivery] = useState<any>(null);
  const [isDetailsDialogOpen, setIsDetailsDialogOpen] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [sortBy, setSortBy] = useState<'aging' | 'poDate' | 'deliveryDate'>('aging');
  ...
  const filteredDeliveries = useMemo(() => {
    ...
    if (currentSubModule === 'delivery-pending') {
      ...
      filtered = filtered.filter(item => item.deliveryStatus === 'Parcel Delivered');
  ...
  }, [deliveryItems, currentSubModule, searchTerm, sortBy, sortOrder, dateFrom, dateTo]);
  ...
  const stats = useMemo(() => {
    const pending = deliveryItems.filter(item => item.deliveryStatus === 'Pending');
    const parcelDelivered = deliveryItems.filter(item => item.deliveryStatus === 'Parcel Delivered');
    const delivered = deliveryItems.filter(item => item.deliveryStatus === 'Delivered');
    
    const totalRevenue = deliveryItems.reduce((sum, item) => sum + (item.totalAmount || 0), 0);
    const advanceCollected = deliveryItems.reduce((sum, item) => sum + (item.advance || 0), 0);
    const balancePending = deliveryItems.reduce((sum, item) => sum + (item.balance || 0), 0);
    const avgAging = deliveryItems.length > 0 
      ? Math.round(deliveryItems.reduce((sum, item) => sum + item.aging, 0) / deliveryItems.length)
      : 0;
  ...
  const handleSaveEdit = () => {
    ...
    const aging = editedData.deliveryDate && editedData.poReceivedDate
      ? Math.floor((new Date(editedData.deliveryDate).getTime() - new Date(editedData.poReceivedDate).getTime()) / (1000 * 60 * 60 * 24))
      : editedData.aging;
  ...
  const handleAddDelivery = () => {
    ...
    const advancePercentage = formData.totalAmount > 0 && formData.advance > 0
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'Parcel Delivered':
        return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">Parcel Delivered</Badge>;
      case 'Delivered':
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl text-orange-900 mt-1">{stats.pending}</h3>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl text-blue-900 mt-1">{stats.parcelDelivered}</h3>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl text-green-900 mt-1">{stats.delivered}</h3>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl text-purple-900 mt-1">{stats.avgAging} days</h3>
              </div>
              ...
              <div className="flex items-center gap-2 border border-gray-200 rounded-lg p-2 bg-white">
                ...
                <Popover open={isDateFromOpen} onOpenChange={setIsDateFromOpen}>
                  <PopoverTrigger asChild>
                    ...
                    >
                      {dateFrom ? format(dateFrom, "dd MMM yyyy") : <span className="text-gray-500">From Date</span>}
                    </Button>
                  </PopoverTrigger>
                ...
                <Popover open={isDateToOpen} onOpenChange={setIsDateToOpen}>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className="w-36 justify-start text-left font-normal border-0 p-0 h-auto hover:bg-transparent"
                    >
                      {dateTo ? format(dateTo, "dd MMM yyyy") : <span className="text-gray-500">To Date</span>}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <CalendarComponent
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-xl">
                  {currentSubModule === 'delivery-pending' && 'Delivery Pending'}
                  {currentSubModule === 'parcel-delivered' && 'Parcel Delivered'}
                  {currentSubModule === 'delivered' && 'Delivered'}
                  {!currentSubModule && 'All Deliveries'}
                </CardTitle>
                <p className="text-sm text-gray-600 mt-1">
                  View and manage delivery records • {filteredDeliveries.length} Total Records
                </p>
              </div>
            </div>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">PO Received</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Delivery Date</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Aging</th>
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="text-sm font-medium text-gray-900">{delivery.projectCode}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          <div className="text-sm font-medium text-gray-900">{delivery.productName}</div>
                          <div className="text-sm text-gray-500">{delivery.brandName} • {delivery.categoryName}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{delivery.poNumber}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{formatDate(delivery.poReceivedDate)}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {delivery.deliveryDate ? formatDate(delivery.deliveryDate) : formatDate(delivery.expectedDeliveryDate)}
                        </div>
                        {!delivery.deliveryDate && delivery.expectedDeliveryDate && (
                          <div className="text-xs text-gray-500">Expected</div>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className={`text-sm font-medium ${getAgingColor(delivery.aging)}`}>
                          {delivery.aging} days
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{delivery.quantity.toLocaleString()}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
              ...
              <div className="flex items-center gap-2">
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                  <Button
                    ...
                    className={currentPage === page ? "bg-[#0c9dcb] hover:bg-[#0a8bb5]" : ""}
                  >
                    {page}
                  </Button>
                ))}
          ...
          <div className="sticky top-0 z-50 px-8 py-6 bg-gradient-to-r from-gray-50 via-white to-gray-50 border-b border-gray-200 shadow-sm">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-6">
                <div className="w-14 h-14 bg-gradient-to-br from-[#0c9dcb] to-[#0a8bb5] rounded-xl flex items-center justify-center shadow-lg">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {selectedDelivery && getStatusBadge(selectedDelivery.deliveryStatus)}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-3">
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white border-2 border-gray-200 rounded-xl p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      <Label className="text-sm text-gray-600">Project Code</Label>
                      {isEditing ? (
                        <Input
                          value={editedData.projectCode}
                          onChange={(e) => setEditedData({ ...editedData, projectCode: e.target.value })}
                          className="mt-1"
                        />
                      ) : (
                        <p className="text-sm text-gray-900 mt-1">{selectedDelivery.projectCode}</p>
                      )}
                    </div>
                    <div>
                      ...
                      {isEditing ? (
                        ...
                        <p className="text-sm text-gray-900 mt-1">{selectedDelivery.poNumber}</p>
                      )}
                    </div>
                    <div>
                      ...
                      {isEditing ? (
                        ...
                        <p className="text-sm text-gray-900 mt-1">{selectedDelivery.productName}</p>
                      )}
                    </div>
                    <div>
                      ...
                      {isEditing ? (
                        ...
                        <p className="text-sm text-gray-900 mt-1">{selectedDelivery.brandName}</p>
                      )}
                    </div>
                    <div>
                      ...
                      {isEditing ? (
                        ...
                        <p className="text-sm text-gray-900 mt-1">{selectedDelivery.categoryName}</p>
                      )}
                    </div>
                    <div>
                      ...
                      {isEditing ? (
                        <Input
                          ...
                          onChange={(e) => setEditedData({ ...editedData, quantity: parseInt(e.target.value) || 0 })}
                          className="mt-1"
                        />
                      ) : (
                        <p className="text-sm text-gray-900 mt-1">{selectedDelivery.quantity.toLocaleString()}</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white border-2 border-gray-200 rounded-xl p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      <Label className="text-sm text-gray-600">PO Received Date</Label>
                      {isEditing ? (
                        <Input
                          type="date"
                          value={editedData.poReceivedDate}
                          onChange={(e) => setEditedData({ ...editedData, poReceivedDate: e.target.value })}
                          className="mt-1"
                        />
                      ) : (
                        <p className="text-sm text-gray-900 mt-1">{formatDate(selectedDelivery.poReceivedDate)}</p>
                      )}
                    </div>
                    <div>
                      ...
                      {isEditing ? (
                        ...
                        <p className="text-sm text-gray-900 mt-1">{formatDate(selectedDelivery.expectedDeliveryDate)}</p>
                      )}
                    </div>
                    <div>
                      ...
                      {isEditing ? (
                        <Input
                          ...
                          onChange={(e) => setEditedData({ ...editedData, deliveryDate: e.target.value })}
                          className="mt-1"
                        />
                      ) : (
                        <p className="text-sm text-gray-900 mt-1">{formatDate(selectedDelivery.deliveryDate) || '-'}</p>
                      )}
                    </div>
                  </div>
                  <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-sm font-medium ${getAgingColor(selectedDelivery.aging)}`}>
                    ...
                    <div>
                      ...
                      {isEditing ? (
                        <Select
                          ...
                          onValueChange={(value) => setEditedData({ ...editedData, deliveryStatus: value })}
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="Parcel Delivered">Parcel Delivered</SelectItem>
                  ...
                  <div className="bg-white border-2 border-gray-200 rounded-xl p-6">
                    ...
                    <div>
                      ...
                      {isEditing ? (
                        ...
                        <p className="text-lg text-gray-900 mt-1">{formatCurrency(selectedDelivery.totalAmount || 0)}</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white border-2 border-gray-200 rounded-xl p-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">Shipping & Customer Information</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      {isEditing ? (
                        ...
                        <p className="text-sm text-gray-900 mt-1">{selectedDelivery.trackingNumber || '-'}</p>
                      )}
                    </div>
                    <div>
                      ...
                      {isEditing ? (
                        <Input
                          ...
                          onChange={(e) => setEditedData({ ...editedData, courierService: e.target.value })}
                          className="mt-1"
                        />
                      ) : (
                        <p className="text-sm text-gray-900 mt-1">{selectedDelivery.courierService || '-'}</p>
                      )}
                    </div>
                    <div>
                      ...
                      {isEditing ? (
                        ...
                        <p className="text-sm text-gray-900 mt-1">{selectedDelivery.customerName || '-'}</p>
                      )}
                    </div>
                    <div>
                      <Label className="text-sm text-gray-600">Customer Contact</Label>
                      {isEditing ? (
                        <Input
                          value={editedData.customerContact || ''}
                          onChange={(e) => setEditedData({ ...editedData, customerContact: e.target.value })}
                          className="mt-1"
                        />
                      ) : (
                        <p className="text-sm text-gray-900 mt-1">{selectedDelivery.customerContact || '-'}</p>
                      )}
                    </div>
                    <div className="md:col-span-2">
                      ...
                      {isEditing ? (
                        ...
                        <p className="text-sm text-gray-900 mt-1">{selectedDelivery.deliveryAddress || '-'}</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white border-2 border-gray-200 rounded-xl p-6">
                  ...
                  <div>
                    {isEditing ? (
                      ...
                      <p className="text-sm text-gray-900">{selectedDelivery.remarks || 'No remarks'}</p>
                    )}
                  </div>
                </div>
              </div>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddDelivery} className="bg-[#0c9dcb] hover:bg-[#0a8bb5]">
              Add Delivery
            </Button>
          </DialogFooter>
        </DialogContent>
