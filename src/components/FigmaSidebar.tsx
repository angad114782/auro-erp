...
export function FigmaSidebar({ currentModule, currentSubModule, onModuleChange, onCollapseChange }: SidebarProps) {
  ...
  const [expandedModules, setExpandedModules] = useState<string[]>(['master-data', 'rd-management', 'production', 'inventory', 'delivery']);
  ...
  const modules: Module[] = [
    ...
    },
    {
      ...
      subModules: [
        { id: 'delivery-pending', name: 'Delivery Pending', icon: <Clock className="w-4 h-4" /> },
        { id: 'parcel-delivered', name: 'Parcel Delivered', icon: <Package className="w-4 h-4" /> },
        { id: 'delivered', name: 'Delivered', icon: <CheckCircle className="w-4 h-4" /> }
      ]
    },
    {
      id: 'inventory',
